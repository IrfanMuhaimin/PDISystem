...
export default function Login({ navigation, onLoginSuccess }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('userType', decodedToken.type || 'unknown'); // Store type even if unknown
      console.log('Login successful. Stored Token:', data.token);
      console.log('Stored User Type:', decodedToken.type || 'unknown');
      if (onLoginSuccess) {
        onLoginSuccess(); // Trigger App.js state update and (now implicit) navigation
      }
    } catch (error) {
...
const localStyles = StyleSheet.create({
    ...
    loginButtonSmall: {
      ...
      // Use commonStyles actionButton for base styles (bg, radius, etc.)
    },
    loginButtonTextSmall: {
       fontSize: FONT_SIZES.large,
       // Inherit color, weight from commonStyles.actionButtonText
    }
});