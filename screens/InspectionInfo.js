...
export default function InspectionInfo({ navigation }) {
    ...
    useEffect(() => {
        const fetchJobCardDetails = async () => {
            ...
            setIsLoadingDetails(true); setFetchError(null); setDetailedVehicleData(null);
            try {
                const token = await AsyncStorage.getItem('authToken'); if (!token) throw new Error("Auth token missing.");
                const url = `${JOB_CARD_DETAIL_ENDPOINT}/${chassisNo}`; console.log(`Fetching Job Card Details: ${url}`);
                ...
                if (!response.ok) {
                    ...
                    if (response.status === 404) { errorMsg = `Job card not found for Chassis: ${chassisNo}. (${response.status})`; }
                    else { try { const eb = await response.json(); errorMsg = eb.message || eb.error || errorMsg; } catch (e) {/*ignore*/ } }
            ...
            } catch (error) { console.error("[InspectionInfo] Fetch Job Card Error:", error); setFetchError(error.message || "An unknown error occurred."); }
    ...
    const { groupedItemsForDisplay, sortedSectionsForDisplay, groupingError } = useMemo(() => {
        ...
        try {
            ...
            console.log("[InspectionInfo useMemo - groupedItems] Grouping successful.");
            return { groupedItemsForDisplay: grouped, sortedSectionsForDisplay: sortedKeys, groupingError: null };
        } catch (err) { console.error("[InspectionInfo useMemo - groupedItems] Grouping Error:", err); return { groupedItemsForDisplay: {}, sortedSectionsForDisplay: [], groupingError: "Grouping failed." }; }
    ...
    const { sectionSummary, sectionSummaryError } = useMemo(() => {
        console.log("[InspectionInfo useMemo - sectionSummary] Calculating section summary from allItems...");
        // Uses the modified `allItems` derived from `data.sections`
        if (!allItems || isLoadingDetails || fetchError) return { sectionSummary: [], sectionSummaryError: null };
        const summaryMap = {};
        try {
            allItems.forEach(item => {
                if (!item?.id) return;
                const sectionName = getSectionNameFromNumber(item.section); // Uses helper function
                if (!summaryMap[sectionName]) summaryMap[sectionName] = { section: sectionName, totalDefectsReported: 0, rectifiedCount: 0 };
                // Check if item failed inspection (pass is false) AND defect array exists
                const hadDefect = item.hasOwnProperty('defect') && Array.isArray(item.defect) && item.pass === false;
                if (hadDefect) {
                    ...
                    if (isItemRectified(item)) { // Uses helper function
                        summaryMap[sectionName].rectifiedCount++;
                    }
                }
            });
            // Filter out sections with no reported defects and sort - Unchanged
            const summaryArray = Object.values(summaryMap).filter(s => s.totalDefectsReported > 0);
            const order = ['A', 'B', 'C', 'D', 'E', 'F', 'Others'];
            ...
            console.log("[InspectionInfo useMemo - sectionSummary] Summary calculation successful.");
            return { sectionSummary: summaryArray, sectionSummaryError: null };
        } catch (err) { console.error("[InspectionInfo useMemo - sectionSummary] Summary Error:", err); return { sectionSummary: [], sectionSummaryError: "Summary calculation failed." }; }
    ...
    const handleOpenImageModal = () => { const images = detailedVehicleData?.images || []; if (images.length > 0) { setCurrentImageIndex(0); setImageLoadingInModal(true); setImageModalVisible(true); } else { Alert.alert("No Images", "No defect location images found for this vehicle."); } };
    ...
    const handleConfirmPdiFromModal = async () => {
        ...
        try {
            const token = await AsyncStorage.getItem('authToken'); if (!token) throw new Error("Token missing.");
            const endpoint = `${PDI_APPROVAL_BASE_ENDPOINT}/${currentChassisNo}`; console.log("API:", endpoint);
            const body = { approval: pdiDecisionToConfirm }; console.log("Body:", body);
            ...
            if (!response.ok) {
                ...
                try { const ed = await response.json(); errorMsg = ed.message || ed.error || `Status ${response.status}`; } catch (e) {/*ignore*/}
                if (response.status === 404) errorMsg += `\n(Check API endpoint/method)`;
                throw new Error(errorMsg);
            }
            const result = await response.json(); console.log("Success:", result);
            Alert.alert("Success", `Inspection status updated to ${pdiDecisionToConfirm} for ${currentChassisNo}.`);
            handleClosePdiConfirmModal();
            navigation.goBack(); // Go back after successful submission
        } catch (error) { console.error("API Error:", error); Alert.alert("Submission Failed", `Could not update status.\nError: ${error.message}`); }
        finally { setIsConfirmingPdi(false); }
    };
                ...
                <View style={localStyles.viewImageButtonContainer}>
                     <TouchableOpacity
                        style={[
                            ...
                            (images.length === 0 || imageLoadingInModal) && localStyles.disabledButton // Apply disabled style
                        ]}
                        ...
                        sectionSummary.map(summary => (
                            <View key={summary.section} style={localStyles.sectionSummaryItem}>
                                ...
                                <View style={localStyles.summaryDetailRow}>
                                    <Text style={localStyles.summaryDetailLabel}>Defects Reported</Text>
                                    <Text style={localStyles.summaryDetailSeparator}>:</Text>
                                    <Text style={localStyles.summaryDetailValue}>{summary.totalDefectsReported}</Text>
                                </View>
                                <View style={localStyles.summaryDetailRow}>
                                    <Text style={localStyles.summaryDetailLabel}>Rectified</Text>
                                    <Text style={localStyles.summaryDetailSeparator}>:</Text>
                                    <Text style={localStyles.summaryDetailValue}>{summary.rectifiedCount}</Text>
                                </View>
                            </View>
                        ))
                    ) : (
                        <Text style={commonStyles.noDataText}>No defects were reported during inspection.</Text>
                    )}
                </View>
                                ...
                                return (
                                    <View key={`${item.id}-${index}`} style={localStyles.summaryItem}>
                                        ...
                                        <View style={localStyles.detailsContainer}>
                                            ...
                                            <View style={localStyles.detailRow}><Text style={localStyles.detailLabel}>Defect Severity</Text><Text style={localStyles.detailSeparator}>:</Text><Text style={localStyles.detailValue}>{firstDefect.severity || 'N/A'}</Text></View>
...
const localStyles = StyleSheet.create({
    ...
    modalHeader: { fontSize: FONT_SIZES.xlarge + 2, fontWeight: 'bold', color: COLORS.white, marginBottom: MARGIN.small, textAlign: 'center', },
    ...
    imageSelector: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', maxWidth: MODAL_IMAGE_WIDTH + 40, marginBottom: MARGIN.medium, alignItems: 'center', },
    ...
    imageContainer: { width: MODAL_IMAGE_WIDTH, height: MODAL_IMAGE_HEIGHT, position: 'relative', justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.black, borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: COLORS.grey, marginBottom: MARGIN.medium, },
    image: { width: '100%', height: '100%', },
    closeButton: { marginTop: MARGIN.large, backgroundColor: COLORS.danger, paddingVertical: PADDING.medium, paddingHorizontal: PADDING.xlarge, borderRadius: 8, },
    closeText: { fontWeight: 'bold', color: COLORS.white, fontSize: FONT_SIZES.medium, },
    disabledButton: { opacity: 0.5, },
    loadingOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.6)', justifyContent: 'center', alignItems: 'center', zIndex: 10, borderRadius: 8, },
    modalLoadingText: { marginTop: MARGIN.small, color: COLORS.white, fontSize: FONT_SIZES.medium, },
    // --- Footer Styles ---
    footerActionContainer: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: PADDING.small, paddingHorizontal: PADDING.small, borderTopWidth: 1, borderTopColor: COLORS.footer, backgroundColor: COLORS.footer, },
