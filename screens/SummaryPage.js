...
export default function SummaryPage({ navigation }) {
  ...
  const [selectedImageKeyForModal, setSelectedImageKeyForModal] =
    useState("vehicle1");
  ...
  const summary = useMemo(() => {
    ...
    try {
      return Object.entries(checklist).map(([section, items]) => {
        ...
        return {
          section,
          checkedCount: validItems.filter((item) => item?.checked).length,
          defectCount: validItems.filter((item) => item?.defect).length,
        };
      });
    } catch (error) {
      console.error("[Memo: summary] Error:", error);
      return [];
    }
  }, [checklist]);
    ...
    try {
      return Object.entries(checklist).flatMap(([sectionKey, items]) => {
        ...
        return validItems
          ...
          .map((item) => {
            const isBatteryItem =
              item.name === BATTERY_ITEM_NAME &&
              sectionKey === BATTERY_ITEM_SECTION;
      ...
      console.error("[Memo: defectSummary] Error:", error);
      return [];
    }
  }, [checklist]);
  ...
  useEffect(() => {
    if (isSubmitting) {
      console.log(
        "[SupervisorFetch - SummaryPage] Skipping fetch: isSubmitting is true."
      );
      return;
    }
    let isMounted = true;
    ...
    const fetchAndFilterSupervisors = async () => {
      ...
      try {
        ...
        if (!token) {
          console.error("[SupervisorFetch - SummaryPage] Auth token missing.");
        ...
        if (!response.ok) {
          ...
          console.error(
            `[SupervisorFetch - SummaryPage] Fetch Error ${status}:`,
            txt
          );
          if (status === 403) throw new Error("Permission denied (403).");
          throw new Error(
            `Fetch users failed (${status}). Server response: ${txt.substring(
              0,
              100
            )}...`
          );
        ...
        if (!Array.isArray(data)) {
          console.error(
            "[SupervisorFetch - SummaryPage] Invalid data format received, expected array:",
            data
          );
          throw new Error("Invalid data format received for users.");
        }
        const filtered = data.filter(
          (u) =>
            u &&
            typeof u.type === "string" &&
            u.type.toLowerCase() === "supervisor"
        );
        console.log(
          `[SupervisorFetch - SummaryPage] Found ${filtered.length} supervisors.`
        );
        if (isMounted) {
          setSupervisors(filtered);
          setSupervisorFetchError(null);
          console.log(
            "[SupervisorFetch - SummaryPage] State updated with supervisors."
          );
        } else {
          console.log(
            "[SupervisorFetch - SummaryPage] Component unmounted before state update."
          );
        }
      } catch (error) {
        if (isMounted) {
          console.error(
            "[SupervisorFetch - SummaryPage] Catch Block Error:",
            error
          );
          setSupervisorFetchError(error.message || "Load failed.");
          setSupervisors([]);
          console.log(
            "[SupervisorFetch - SummaryPage] State updated with fetch error."
          );
        } else {
          console.log(
            "[SupervisorFetch - SummaryPage] Component unmounted before error state update."
          );
        }
      } finally {
        if (isMounted) {
          setIsLoadingSupervisors(false);
          console.log("[SupervisorFetch - SummaryPage] Loading set false.");
        } else {
          console.log(
            "[SupervisorFetch - SummaryPage] Component unmounted before finally block."
          );
        }
      }
    };
    ...
    return () => {
      isMounted = false;
      console.log(
        "[SupervisorFetch - SummaryPage] Unmounting, fetch cancelled/ignored."
      );
    };
  }, [isSubmitting]);
  // --- End useEffect ---
  ...
  const getImageSource = (selectedImageKey) =>
    baseImages[selectedImageKey] || baseImages["vehicle1"];
  ...
  const handlePreviousImage = () => {
    const currentIndex = imageKeys.indexOf(selectedImageKeyForModal);
    const previousIndex =
      (currentIndex - 1 + imageKeys.length) % imageKeys.length;
    setSummaryImageLayout(null);
  ...
  const onSummaryImageLayout = useCallback(
    (event) => {
      console.log("[Summary Layout] onLayout fired");
      const { width, height } = event.nativeEvent.layout;
      if (width > 0 && height > 0) {
        if (
          !summaryImageLayout ||
          summaryImageLayout.width !== width ||
          summaryImageLayout.height !== height
        ) {
          console.log(
            `[Summary Layout] *** Setting layout state: ${width}x${height} ***`
          );
          setSummaryImageLayout({ width, height });
        }
      } else {
        console.warn(`[Summary Layout] Invalid dimensions: ${width}x${height}`);
      }
    },
    [summaryImageLayout]
  );
  ...
  const defectsForCurrentImage = useMemo(() => {
    ...
    return defectSummary.filter(
      (defect) => defect.selectedImage === currentImageKey
    );
  }, [defectSummary, currentImageKey]);
  const renderableMarks = useMemo(() => {
    console.log(
      "[Memo] Calculating renderable marks for DISPLAY. Layout:",
      summaryImageLayout ? "Yes" : "No",
      "Defect count:",
      defectsForCurrentImage.length
    );
    if (
      !summaryImageLayout ||
      !Array.isArray(defectsForCurrentImage) ||
      defectsForCurrentImage.length === 0
    ) {
      return [];
    }
    const marks = [];
    defectsForCurrentImage.forEach((defect, index) => {
      ...
      defectMarks.forEach((mark, j) => {
        if (
          ...
          console.warn(
            `[Mark Calc Display] Expected {nx, ny} but found:`,
            mark
          );
        }
      });
  ...
  const handleFinish = async () => {
    if (!selectedSupervisor) {
      Alert.alert(
        "Supervisor Required",
        "Please select the supervisor who checked the final inspection."
      );
      return;
    }
    ...
    try {
      ...
      if (staffToken) {
        ...
        staffUsername = decoded?.username || "unknown";
      }
    } catch (e) {
      console.error("Error decoding token:", e);
    }

    const supervisorObject = Array.isArray(supervisors)
      ? supervisors.find((sv) => sv.id === selectedSupervisor)
      : null;
    const supervisorUsername =
      supervisorObject?.username || "unknown_supervisor";
    ...
    const requestBody = {
      ...
      start_time:
        carInfo.startTime?.toISOString().slice(0, 19).replace("T", " ") ||
    ...
    Object.entries(checklist || {}).forEach(([sectionKey, itemsInSection]) => {
      ...
      validItems.forEach((item) => {
        ...
        if (item.defect && item.defectDetails) {
          ...
          const imageId =
            parseInt(selectedImage.replace("vehicle", "") || "1", 10) || 1;
          ...
          const firstValidMark = normalizedMarksFromContext.find(
            (mark) =>
              mark && typeof mark.nx === "number" && typeof mark.ny === "number"
          );
          if (firstValidMark) {
            formattedMarkObject = {
              x: firstValidMark.nx,
        ...
        if (isBatteryItem) {
          ...
          if (
            item.defect &&
            item.defectDetails &&
            typeof item.defectDetails.measurementValue === "number"
          ) {
    ...
    console.log(
      '--- Submitting Checklist Payload (Top-level "value" for battery) ---'
    );
    ...
    try {
      ...
      const response = await fetch(JOBCARD_SUBMIT_ENDPOINT, {
        method: "POST",
        headers: {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <Text style={[commonStyles.pageHeader, { marginBottom: MARGIN.small }]}>
          PDI Summary
        </Text>

        {/* Car Info (Original) */}
        <View style={themedStyles.infoBox}>
          <Text style={themedStyles.carInfoText}>
            <Text style={themedStyles.carInfoLabel}>Chassis:</Text>{" "}
            {carInfo?.chassis_no || "N/A"}
          </Text>
          <Text style={themedStyles.carInfoText}>
            <Text style={themedStyles.carInfoLabel}>Model:</Text>{" "}
            {carInfo?.model || "N/A"}
          </Text>
          <Text style={themedStyles.carInfoText}>
            <Text style={themedStyles.carInfoLabel}>Engine:</Text>{" "}
            {carInfo?.engine_no || "N/A"}
          </Text>
          <Text style={themedStyles.carInfoText}>
            <Text style={themedStyles.carInfoLabel}>Color:</Text>{" "}
            {carInfo?.colour_code || "N/A"}
          </Text>
          <Text style={themedStyles.carInfoText}>
            <Text style={themedStyles.carInfoLabel}>Entry:</Text>{" "}
            {carInfo?.entry_date || "N/A"}
          </Text>
        </View>
        ...
        <View
          style={[
            originalStyles.pickerContainer,
            (isLoadingSupervisors ||
              !!supervisorFetchError ||
              supervisors.length === 0) &&
              themedStyles.pickerContainerDisabled,
          ]}
        >
          {isLoadingSupervisors ? (
            ...
            <Text
              style={[themedStyles.pickerPlaceholder, commonStyles.errorText]}
            >
              Error: {supervisorFetchError}
            </Text>
          ) : supervisors.length === 0 ? (
            <Text style={themedStyles.pickerPlaceholder}>
            ...
            >
              <Picker.Item
                ...
                value={null}
                style={themedStyles.pickerPlaceholderItem}
                enabled={false}
              />
              {supervisors.map((sv) => (
                <Picker.Item
        ...
        <View style={originalStyles.summaryContainerView}>
          {summary.map(({ section, checkedCount, defectCount }, index) => (
            <View
              key={section}
              style={[
                originalStyles.summaryRow,
                index === 0 && themedStyles.firstSummaryRow,
              ]}
            >
              <View style={originalStyles.summaryColumnSection}>
                <Text style={originalStyles.summaryText}>
                  Section {section}
                </Text>
              </View>
              <View style={originalStyles.summaryColumnCount}>
                <Text style={originalStyles.checkedText}>
                  Checked: {checkedCount}
                </Text>
              </View>
              <View style={originalStyles.summaryColumnCount}>
                <Text style={originalStyles.defectText}>
                  Defects: {defectCount}
                </Text>
              </View>
            </View>
        ...
        {defectSummary.length > 0 && (
          <View style={originalStyles.viewCenter}>
            ...
            >
              <Text style={originalStyles.viewImageText}>
          ...
          <>
            ...
            <View style={originalStyles.defectContainerView}>
              {defectSummary.map((defect, index) => (
                <View
                  key={`${defect.section}-${defect.name}-${index}`}
                  style={originalStyles.defectBox}
                >
                  <Text style={originalStyles.defectItem}>
                    <Text style={originalStyles.boldText}>
                      Section {defect.section} - {defect.name}
                    </Text>
                  </Text>
                  <Text style={originalStyles.defectDetail}>
                    {defect.category} - {defect.type}
                  </Text>
                  <Text style={originalStyles.defectDetail}>
                  ...
                  {defect.isBatteryItem && defect.measurementValue !== null && (
                    <Text style={originalStyles.defectDetail}>
                      Measured Value: {defect.measurementValue}
                      {typeof defect.measurementValue === "number" ? "V" : ""}
                    </Text>
                  )}
                  <Text style={originalStyles.remarksText}>
                    Remarks: {defect.remarks}
                  </Text>
                  {defect.selectedImage &&
                    defect.marks &&
                    defect.marks.length > 0 && (
                      <Text style={originalStyles.defectDetail}>
                        Marked on: {defect.selectedImage}
                      </Text>
                    )}
                </View>
              ))}
      ...
      <View style={originalStyles.footer}>
        <TouchableOpacity
          style={[
            originalStyles.buttonBack,
            isSubmitting && originalStyles.buttonDisabled,
          ]}
        ...
        >
          <Text
            style={[
              originalStyles.buttonText,
              originalStyles.buttonBackText,
              isSubmitting && originalStyles.buttonTextDisabled,
            ]}
          >
            Back
          </Text>
        ...
        <View style={originalStyles.modalContainer}>
          ...
          <View style={originalStyles.imageSelector}>
            <TouchableOpacity
              ...
              style={originalStyles.imageButton}
          ...
          <View style={originalStyles.imageWrapper}>
            ...
            {!summaryImageLayout && (
              <View style={originalStyles.loadingOverlay}>
                ...
                <Text style={originalStyles.loadingText}>
                  Calculating Layout...
                </Text>
              </View>
            )}
            {summaryImageLayout &&
              renderableMarks.map((mark) => (
                <View
                  key={mark.key}
                  style={[
                    originalStyles.markerContainer,
                    { left: mark.pixelX, top: mark.pixelY },
                  ]}
                >
                  {" "}
                  {mark.location === "Interior" && (
                    <View style={originalStyles.circle} />
                  )}{" "}
          ...
          <TouchableOpacity
            style={originalStyles.closeButton}
            onPress={() => {
              setImageModalVisible(false);
            }}
          >
      ...
      >
        <View style={themedStyles.successModalContainer}>
          <View style={themedStyles.successModalContent}>
            ...
            <Text style={themedStyles.successModalText}>
              Job Card has been submitted.
            </Text>
            <Text style={themedStyles.successModalTimeText}>
              Entry Time:{" "}
              {carInfo.startTime ? formatTime(carInfo.startTime) : "N/A"}
            </Text>
            <Text style={themedStyles.successModalTimeText}>
              Exit Time: {carInfo.endTime ? formatTime(carInfo.endTime) : "N/A"}
            </Text>
            <TouchableOpacity
              style={[
                commonStyles.actionButton,
                themedStyles.successModalButton,
              ]}
              onPress={() => {
...
const originalStyles = StyleSheet.create({
  ...
  loadingOverlay: {
    ...
    alignItems: "center",
    zIndex: 10,
  }, // This is the general loading overlay
  loadingText: {
