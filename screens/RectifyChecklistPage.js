            ...
            defectItems.forEach((item) => {
                ...
                if (typeof sectionValue === 'number') {
                    ...
                    if (sectionValue.length === 1 && /^[A-F]$/i.test(sectionValue)) {
                        ...
                        if (!isNaN(sectionNumber) && sectionNumber >= 1 && sectionNumber <= 7) {
                            mappedSectionKey = getSectionLetter(sectionNumber);
                        }
                    }
                } else {
                     console.warn(`[Grouping Debug] -> Section value has unexpected type: ${typeof sectionValue} for Item ID ${item.id}`);
                }
        ...
        defectItems.forEach(item => {
            ...
            defectsArray.forEach((defectInfo, defectIndex) => {
                ...
                marksData.forEach((mark, markIndex) => {
                    ...
                    if (!isValidMark) {
                        console.warn(`[Memo Marks] Invalid mark format for item ${item.id}, defect ${defectIndex}, mark ${markIndex}:`, mark);
                        return;
                    }
    ...
    return (
        <ScreenWrapper showHeader={true} showFooter={true} enableScrollView={false} enableKeyboardAvoidingView={Platform.OS === 'ios'} >
            ...
            >
                ...
                <View style={{ height: 40 }} />{/* Scroll bottom padding */}
            </ScrollView>
                      ...
                      <View style={localStyles.modalContainer}>
                         ...
                         <View style={localStyles.imageContainer}>
                             <Image
                                ...
                                onLoadStart={() => { console.log(`Image onLoadStart: ${imageUrl}`); setImageLoading(true); }}
                                onLoad={() => { console.log(`Image onLoad Success: ${imageUrl}`); setImageLoading(false); }}
                                onError={(e) => { console.error(`Image Load Error: ${imageUrl}`, e.nativeEvent.error); setImageLoading(false); Alert.alert("Image Error", `Failed to load image: ${currentImageData.description || imageUrl}. \nError: ${e.nativeEvent.error}`); }}
                                resizeMode="contain"
                             ...
                             {modalImageLayout && marksForCurrentImage.length > 0 && (
                                 <View style={StyleSheet.absoluteFillObject} pointerEvents="none">
                                     {marksForCurrentImage.map(mark => (
                                         <View
                                            key={mark.id}
                                            style={[ localStyles.markerContainer, { left: mark.pixelX, top: mark.pixelY } ]}
                                         >
...
const localStyles = StyleSheet.create({
    ...
    centeredMessageContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: PADDING.large, marginTop: MARGIN.xlarge, },
    subHeader: { fontSize: FONT_SIZES.large, textAlign: 'center', marginBottom: MARGIN.xsmall, color: COLORS.secondary, fontWeight: '600', },
    carInfo: { fontSize: FONT_SIZES.medium, marginBottom: MARGIN.medium, textAlign: 'center', color: COLORS.grey, },
    viewImageButtonContainer: { alignItems: 'center', marginVertical: MARGIN.medium, },
    viewImageButton: { backgroundColor: COLORS.primary, paddingVertical: PADDING.small, paddingHorizontal: PADDING.medium, borderRadius: 8, alignItems: 'center', elevation: 2, shadowColor: COLORS.black, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 1.5, },
    disabledButton: { backgroundColor: COLORS.disabled, opacity: 0.7, elevation: 0, },
    viewImageButtonText: { color: COLORS.white, fontWeight: 'bold', fontSize: FONT_SIZES.medium, },
    sectionContainer: { marginBottom: MARGIN.medium, backgroundColor: COLORS.white, borderRadius: 8, elevation: 1, shadowColor: COLORS.grey, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.15, shadowRadius: 2, overflow: 'hidden', },
    sectionHeader: { fontSize: FONT_SIZES.large, fontWeight: 'bold', color: COLORS.secondary, backgroundColor: COLORS.primaryLight, paddingVertical: PADDING.small, paddingHorizontal: PADDING.medium, },
    item: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'transparent', paddingVertical: PADDING.medium, paddingHorizontal: PADDING.medium, borderBottomWidth: 1, borderBottomColor: COLORS.divider, },
    itemDetails: { flex: 1, marginRight: MARGIN.medium, },
    itemName: { fontSize: FONT_SIZES.medium, fontWeight: '600', marginBottom: MARGIN.xsmall, color: COLORS.secondary, lineHeight: FONT_SIZES.medium * 1.3, },
    detailsContainer: { marginLeft: MARGIN.small, marginBottom: MARGIN.xsmall, },
    detailRow: { flexDirection: 'row', marginBottom: MARGIN.xsmall / 2, alignItems: 'flex-start', },
    detailLabel: { fontSize: FONT_SIZES.small, color: COLORS.grey, fontWeight: '600', width: 70, }, // Using fixed width for label alignment
    detailSeparator: { fontSize: FONT_SIZES.small, color: COLORS.grey, fontWeight: '600', marginHorizontal: MARGIN.xsmall / 2, }, // Kept separator
    detailValue: { flex: 1, fontSize: FONT_SIZES.small, color: COLORS.secondary, lineHeight: FONT_SIZES.small * 1.5, }, // Style for value text
    rectifierInfoContainer: { marginTop: MARGIN.small, marginLeft: MARGIN.small, paddingTop: MARGIN.small, borderTopWidth: 1, borderTopColor: COLORS.divider, },
    rectifierInfoText: { fontSize: FONT_SIZES.xsmall, fontStyle: 'italic', color: COLORS.success, marginBottom: MARGIN.xsmall / 2, lineHeight: FONT_SIZES.xsmall * 1.3, },
    remarkText: { fontSize: FONT_SIZES.xsmall, fontStyle: 'italic', color: COLORS.grey, lineHeight: FONT_SIZES.xsmall * 1.3, },
    checkboxStyle: { width: 24, height: 24, },
    errorDetailText: { color: COLORS.danger, fontSize: FONT_SIZES.small, textAlign: 'center', marginTop: MARGIN.small, },
    modalContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.85)', paddingVertical: PADDING.large, paddingHorizontal: PADDING.medium, },
    modalHeader: { fontSize: FONT_SIZES.xlarge, fontWeight: 'bold', color: COLORS.white, marginBottom: MARGIN.small, textAlign: 'center', },
    modalSubHeader: { fontSize: FONT_SIZES.medium, color: COLORS.lightGrey, marginBottom: MARGIN.large, textAlign: 'center', paddingHorizontal: PADDING.small, },
    imageSelector: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', maxWidth: SUMMARY_PAGE_IMAGE_WIDTH + 40, marginBottom: MARGIN.medium, alignItems: 'center', },
    imageButton: { backgroundColor: COLORS.primaryLight, paddingVertical: PADDING.small, paddingHorizontal: PADDING.medium, borderRadius: 5, minWidth: 80, alignItems: 'center', },
    imageButtonText: { fontWeight: 'bold', color: COLORS.secondary, fontSize: FONT_SIZES.medium, },
    imageContainer: { position: 'relative', borderColor: COLORS.grey, borderWidth: 1, marginVertical: MARGIN.medium, justifyContent: 'center', alignItems: 'center', width: '100%', aspectRatio: SUMMARY_PAGE_IMAGE_WIDTH / SUMMARY_PAGE_IMAGE_HEIGHT, maxWidth: SUMMARY_PAGE_IMAGE_WIDTH, maxHeight: SUMMARY_PAGE_IMAGE_HEIGHT, backgroundColor: COLORS.black, borderRadius: 5, overflow: 'hidden', },
    image: { width: '100%', height: '100%', },
    markerContainer: { position: 'absolute', alignItems: 'center', justifyContent: 'center', width: MARKER_SIZE, height: MARKER_SIZE, },
    markerX: { fontSize: FONT_SIZES.large, fontWeight: 'bold', color: COLORS.redinfo, textShadowColor: 'rgba(255, 255, 255, 0.7)', textShadowOffset: { width: 1, height: 1 }, textShadowRadius: 2, },
    markerCircle: { width: MARKER_SIZE * 0.9, height: MARKER_SIZE * 0.9, borderRadius: MARKER_SIZE * 0.45, borderWidth: 2, borderColor: COLORS.warning, position: 'absolute', backgroundColor: 'transparent', },
    closeButton: { marginTop: MARGIN.large, backgroundColor: COLORS.grey, paddingVertical: PADDING.medium, paddingHorizontal: PADDING.xlarge, borderRadius: 8, },
    closeText: { fontWeight: 'bold', color: COLORS.white, fontSize: FONT_SIZES.medium, },
    loadingOverlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 0.6)', justifyContent: 'center', alignItems: 'center', zIndex: 10, borderRadius: 5, },
    loadingText: { marginTop: MARGIN.small, color: COLORS.grey, fontSize: FONT_SIZES.medium, }, // General loading text
    modalLoadingText: { marginTop: MARGIN.small, color: COLORS.white, fontSize: FONT_SIZES.medium, } // Loading text for modal overlay
});
// --- End Styles ---